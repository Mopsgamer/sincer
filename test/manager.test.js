...
describe("Manager", function () {
  ...
  describe("Config loading", function () {
    ...
    it("Virtual cfg", function () {
      ...
      chai.assert.strictEqual(sincer.cfg.records.length, 0);
      chai.assert.throws(function () {
        sincer.cfgSaveToFile();
      });
      chai.assert.throws(function () {
        sincer.cfgParseFromFile();
      });
      chai.assert.throws(function () {
        sincer.cfgLoadFromFile();
      });
      ...
      chai.assert.throws(function () {
        sincer.cfgLoad({ count: 0, records: 1 });
      });
      chai.assert.throws(function () {
        sincer.cfgLoad({ count: 0, records: [{ name: "", since: "" }] });
      });
      chai.assert.throws(function () {
        sincer.cfgLoad({ count: 0, records: [{ name: 1, since: "" }] });
      });
      chai.assert.throws(function () {
        sincer.cfgLoad({ count: 0, records: [{ name: "", since: 1 }] });
      });
      sincer.cfgLoad({
        count: 0,
        records: [{ name: "normal", since: new Date().toJSON() }],
      });
    });
  });
});
