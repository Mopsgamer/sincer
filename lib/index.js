...
export class Manager {
	...
	moveDown(name, count) {
		...
		count ?? (count = 'max-down')
		if (count === 'xdown') count = maxCountDown;
		if (count === 'xup') count = maxCountUp;
		if (count === 'up') count = Math.min(1, maxCountUp);
		if (count === 'down') count = Math.min(1, maxCountDown);
		count = Number(count)
		//#region replacing
		const indexNew = Math.max(0, Math.min(index + count, records.length));
