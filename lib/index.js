...
export const defaultCfgPath = join(rootPath, "sincer.yaml");
 ...
 * @param {boolean} [dim]
 */
export function recordStr(record, color = "blue", dim = false) {
	const recordDate = new Date(record.since);
	const time = (new Date()).getTime() - recordDate.getTime();
	const clr = chalk[color]
	const d = clr('|')
	const str = `${clr('{')} ${record.name} ${d} ${prettyms(time)} ${d} ${chalk.gray(new Date(record.since).toLocaleString())} ${clr('}')}`;
	return dim ? chalk.dim(str) : str;
}
...
export class Manager {
	...
	printRecord(record) {
		return recordStr(record);
	}
	printRecordExists(record) {
		return `'${record.name}' already exists: ${recordStr(record, "green")}`;
	}
	printRecordAdded(record) {
		return `added: ${recordStr(record, "greenBright")}`;
	}
	printRecordRemoved(record) {
		return `removed: ${recordStr(record, "redBright")}`;
	}
	printRecordChanged(record, old) {
		return `changed: ${recordStr(old, "blueBright", true)} -> ${recordStr(record, "blueBright")}`;
	}
	//#endregion
	...
	showAll(name) {
		if (!this.cfg) {
			throw new TypeError(this.printBadCfg());
		}
		...
		if (typeof name === "string") {
			...
			for (const record of filtEntries) {
				list.push(this.printRecord(record));
			}
			return list.join('\n');
		}
		const list = []
	...
	add(name, options) {
		...
		return this.printRecordAdded(record);
	}
	swap(name, name2, mode) {
		...
		let message = ''
		if (mode === "names") {
			message += this.printRecordChanged(record, merge(true, record, {name: record2.name})) + '\n';
			message += this.printRecordChanged(record2, merge(true, record2, {name: record.name}));
			[record.name, record2.name] = [record2.name, record.name];
		} else if (mode === "places") {
			message += this.printRecordChanged(record, record2) + '\n';
			message += this.printRecordChanged(record2, record);
			const temp = {...record2};
	...
	redate(name, newdate) {
		...
		let message = this.printRecordChanged(newrecord, record);
	...
	rename(name, newname) {
		...
		if (similar) {
			throw new TypeError(`can not rename ${recordStr(record)}\n${this.printRecordExists(similar)}`)
		}
		const oldrecord = {...record};
		record.name = newname;
		if (this.path !== null)
			this.cfgSaveToFile();
		return this.printRecordChanged(record, oldrecord);;
	}
	moveUp(name, count) {
		if (!this.cfg) {
			throw new TypeError(this.printBadCfg());
	...
	remove(name) {
		...
		for (let recordIndex = this.findRecordIndex(records, name); recordIndex >= 0; recordIndex = this.findRecordIndex(records, name)) {
			list.push(this.printRecordRemoved(records[recordIndex]))
