...
function isRecordItem(rec) {
	return typeof rec === "object" && rec !== null && !Array.isArray(rec) && "name" in rec && typeof rec.name === "string" && "since" in rec && typeof rec.since === "string" && "locale" in rec && typeof rec.locale === "string";
}
...
class Manager {
	...
	async printRecordExists(record) {
		return `'${record.name}' already exists: ${await recordStr(record, "bgGreen", "green")}`;
	}
	async printRecordAdded(record) {
		return `added: ${await recordStr(record, "bgGreenBright", "greenBright")}`;
	}
	async printRecordRemoved(record) {
		return `removed: ${await recordStr(record, "bgRedBright", "redBright")}`;
	}
	async printRecordChanged(record, old) {
		return `changed: ${await recordStr(old, "bgBlueBright", "blueBright", true)} -> ${await recordStr(record, "bgBlueBright", "blueBright")}`;
	}
	...
	async swap(name, name2, mode) {
		...
		if (!isRecordName(name2)) {
			throw new TypeError('second name is invalid')
		}
		const {records} = this.cfg;
	...
	async redate(name, newdate) {
		...
		const newrecord = createRecord(name, newdate || /* @__PURE__ */ new Date());
	...
	async rename(name, newname) {
		...
		if (!isRecordName(newname)) {
			throw new TypeError('new name is invalid')
		}
		if (name === newname) {
