...
export function createRecord(name, date, color = 0xffffff) {
...
export class Manager {
	...
	findRecordIndex(records, name) {
		...
		if(!matcher) {
			throw new Error('mathcher is undefined, name can be invalid')
		}
	...
	restart(name, newdate) {
		...
		const newsince = createRecord(record.name, newdate || new Date(), record.color).since;
		/**@type {RecordItemFormatted[]}*/
		const recordsFm = structuredClone(records)
