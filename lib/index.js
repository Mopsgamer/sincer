...
/**
 * @param {Map | string | undefined} message
 */
export function throwFail(message) {
  ...
  throw new Error(Array.from(message.entries()).map(([key, value])=> `${key}: ${value}`).join('\n'));
}
...
/** @type {Config<SincerConfig>} */
export const cfg = new Config(join(os.homedir(), '.sincer'))
cfg.setValidator('records', [], Types.array(Types.object()))
cfg.setValidator('count', 0, Types.integer())
...
export class Manager {
  ...
  create(name, options) {
    ...
    throwFail(cfg.failSet('count', count + 1));
